...
async function processRealStripePayment(paymentMethod, plan, customerName, customerEmail) {
    ...
    console.log('🔄 Calling webhook handler to process real payment...');
    
    // Call GitHub Pages webhook handler to create real payment intent
    let paymentResponse = null;
    try {
        const webhookUrl = 'https://gurukullam.github.io/GammaPace-SetUp/stripe-webhook-handler.html';
        
        console.log('🌐 Sending payment request to webhook handler:', {
            url: webhookUrl,
            amount: plan.amount,
        ...
        // Use postMessage to communicate with webhook handler
        const requestId = 'payment_' + Date.now();
        
        // Create iframe to load webhook handler
        const iframe = document.createElement('iframe');
        iframe.src = webhookUrl;
        iframe.style.display = 'none';
        document.body.appendChild(iframe);
        
        // Wait for iframe to load and send payment request
        const paymentPromise = new Promise((resolve, reject) => {
            ...
            const messageHandler = (event) => {
                if (event.origin !== 'https://gurukullam.github.io') return;
                if (event.data.type !== 'PAYMENT_INTENT_RESPONSE') return;
            ...
            // Send payment request after iframe loads
            iframe.onload = () => {
                console.log('✅ Webhook handler loaded, sending payment request...');
